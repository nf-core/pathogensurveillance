...
for (i in seq_along(snp_alignments)) {
    ...
    a_plot <- psminer::make_MSN(snp_alignments[[i]], sample_data)
    if (knitr::is_html_output()) {
        cat(as.character(htmltools::as.tags(a_plot, standalone = TRUE)))
    } else {
        print(a_plot)
...
```{r}
# snp_aln.gi <- DNAbin2genind(snp_alignments[[i]])
...
```{asis div_no_msn, echo = length(snp_trees) >= 3}
There is no MSN to draw, probably because there were too few samples.
More info will be added later.
```
