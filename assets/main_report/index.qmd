...
Add settings used to run Nextflow and the pipeline parameters. (Nik thinks this should go down at the end given taht status is reported above).
...
# ![](favicon.png) Genetic diversity in the group(s)
...
#Martha-I notice SNP tree labels are now revised to include ref accession as prefix, for one dataset (2). Here I (martha), was just playing with how to make informative and simple labels. There may be a more simplified approach. Please revise as needed
...
p <- ggtree(snp_trees_midpoint, size=0.5) + # Adjusted branch size
  ...
  geom_point(data=snp_tree1, aes(x=x, y=y, color=tip_color), show.legend = FALSE, size=3) +
  geom_text(data=snp_tree1, aes(x = x + 0.44 + 0.03, y = y, label = label), size=3, hjust=0, vjust=0.5, fontface="bold") +
...
> To contribute or report bugs please visit our [github repository](https://github.com/nf-core/pathogensurveillance).

Please cite this pipeline in publications as follows: 

{{< bi file-earmark-pdf >}} Foster et al. 2024. PathogenSurveillance: A nf-core pipeline for rapid analysis of pathogen genome data. In preparation.
