...
params:
  inputs: "~/pathogensurveillance/assets/main_report/_test_data/xanthomonas/xan_test_inputs"
execute:
  echo: false
---
...
for (i in seq_along(snp_alignments)) {
  cat(paste0('::: panel-tabset\n### ', names(snp_alignments)[i], '\n\n'))
  cat(':::\n')
  
  if (nrow(snp_alignments[[i]]) > 4) {
    for (threshold_index in seq_along(threshold_options)) {
      threshold <- threshold_options[threshold_index]
      
      cat(paste0('\n#### Relative SNP difference: ', threshold, '\n\n'))
