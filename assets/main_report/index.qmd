...
# Parse reference metadata (Martha)
parse_ref_meta <- function(refseq_path, user_ref_path) {
    # Get metadata for all possible refseq references
    ref_data <- read_tsv(refseq_path, col_types = 'dcccccccccccccccddc')
    
    # Get names of user-defined references
    refs <- strsplit(read_lines(ref_ids_path), split = ';', fixed = TRUE)[[1]]
    
    # Add column for origin of reference
    ref_data$origin = "refseq"
    
    # additional columns needed when tables are combined
    #   - origin: whether each row is a refseq or user-defined
    #   - reference_id: "LastMajorReleaseAccession" for refseq, reference_id for user-defined,
    #   - display_name: 'organism' for refseq and "reference_id" user-defined
    
}


#testing lines of code-just a placeholder for now. 
...
parse_ref_meta(ref_data_path, reformatted_sample_df)
...
#Still fixing non interactive version
psminer::make_phylogeny(core_phylo_path, samp_data, ani_matrix, ref_data, interactive=TRUE)
```
