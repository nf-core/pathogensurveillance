...
params:
    inputs: "/media/fosterz/external_primary/files/projects/work/current/pathogensurveillance/work/aa/385dae139f667d89ffa5880e072316"
execute:
  echo: false
---
...
*Martha-Not sure if this is useful, but one idea.
...
```{r ggtree starting point}
gg <- ggtree(snp_trees_midpoint) %<+% samp_data +
  geom_tiplab(size=2.5) +
  #geom_tippoint(aes(color=factor(year),fill=factor(year)), size=4.5, stroke=0, alpha=0.75) + # commented out to make it run when year is not available
...
samp_data <- map_dfc(samp_data, function(col) ifelse(is.na(col), "unknown", col))
...
strata(snp_genclone) <- samp_data[, c(1:18)]
setPop(snp_genclone) <- ~nusery
