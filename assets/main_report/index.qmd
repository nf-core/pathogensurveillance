...
plot(nj_tree)
##Used static ggtree because I was having trouble rendering phycanvas option with tabular format

# Basic tree visualization
#p <- ggtree(nj_tree)

# Highlight certain labels in green

#sample_ids <- nj_tree$tip.label[nj_tree$tip.label %in% convert_id(samp_data$sample)]

#p <- p + 
  #geom_tiplab(aes(color = label %in% sample_ids), size = 3) +
  #scale_color_manual(values = c("TRUE" = "green", "FALSE" = "black"))

# Print the tree
#print(p)

# Visualize-used phycanvas for now-won't render in tabular format ,but looks nice
...
for (x in name_key[sample_ids]) {
  phycanv_snp <- style_node(phycanv_snp, x, labelcolor = "green", labeltextsize = 30)
}
