...
[see webstie](https://stackoverflow.com/questions/56787941/embedding-dash-plotly-graphs-into-html) to create interactive trees:
<head>
  <!-- Plotly.js -->
...
ani_matrix_format <- as.matrix(ani_matrix, row.names=1, header=TRUE)
#Convert ANI similarity values to distances
dist_matrix <- 1 - ani_matrix_format
nj_tree <- nj(dist_matrix)

#TO discuss-ran into instances where negative branches were output. One idea is to set these negative branches to very small positive values. Tree is simply a means to visualize ANI data before core gene phylogeny is produced. Discuss if there are better solutions.

nj_tree$edge.length[nj_tree$edge.length < 0] <- 1e-5  # or some other small value
nj_tree<-midpoint(nj_tree)
#TODO-adjust how tree is rotted if necessary
plot(nj_tree)

#TODO fix labels like what was done with core gene phylogeny
#convert_id <- function(ids) {
##    gsub(ids, pattern = "[.-]", replacement = "_")
#}
...
#colnames(ani_matrix) <- convert_id(colnames(ani_matrix))
#rownames(ani_matrix) <- colnames(ani_matrix)
#group_ani <- ani_matrix[rownames(ani_matrix) %in% nj_tree$tip.label, colnames(ani_matrix) %in% #nj_tree$tip.label]
...
#TODO label tree tips like core gene phylogeny
```
...
Here we are testing ggtree and ggplotly
