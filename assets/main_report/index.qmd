...
```{r generate-tables}
messages_path <- file.path(params$inputs, "messages.csv")
if (file.exists(messages_path)) {
    messages <- readr::read_csv(messages_path)
} else {
    messages <- tibble(sample_id = character(0), reference_id = character(0), workflow = character(0), level = character(0), message = character(0))
}
