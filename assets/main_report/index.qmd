...
```{r}
core_plots <- core_tree_plot(params$inputs)
```


```{r id_core_phylo, results='asis', eval = length(core_tree_path(params$inputs)) > 0, fig.height = 8}
...
to_render <- c(to_render, paste0('\n::: panel-tabset\n'))
for (factor in names(core_plots)) {
    to_render <- c(to_render, paste0('\n### ', factor, '\n'))
    to_render <- c(to_render, paste0('```{r core_plot_', which(names(core_plots) == factor), ', fig.height = 8}'))
    to_render <- c(to_render, paste0('core_plots[["', factor, '"]]'))
    to_render <- c(to_render, paste0('```'))
}
to_render <- c(to_render, paste0('\n:::\n'))
res = knitr::knit_child(text = to_render, quiet = TRUE)
cat(res, sep = '\n')
# cat(as.character(htmltools::as.tags(core_plots[[1]], standalone = TRUE)))
# htmltools:::knit_print.html(htmltools::as.tags(core_plots[[1]]))
```
...
reference_data <- ref_meta_parsed(params$inputs)
group <- report_group_parsed(params$inputs)
snp_align_paths <- variant_align_path(params$inputs)
...
```{r parse_snp_trees}
snp_tree_paths <- variant_tree_path(params$inputs)
